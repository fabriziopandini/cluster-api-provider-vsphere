...
func (c conversionSubResourceClient) Get(ctx context.Context, obj client.Object, subResource client.Object, opts ...client.SubResourceGetOption) error {
	// FIXME: where this is used?
	// TODO implement me
	panic("implement me")
}

func (c conversionSubResourceClient) Create(ctx context.Context, obj client.Object, subResource client.Object, opts ...client.SubResourceCreateOption) error {
	gvk, err := c.GroupVersionKindFor(obj)
	if err != nil {
		return err
	}
	...
	if !conversionRequired(gvk) {
		return c.internalClient.Status().Create(ctx, obj, subResource, opts...)
	}
	...
	// FIXME: should we convert also subResource
	if err := c.internalClient.Status().Create(ctx, spokeObj, subResource, opts...); err != nil {
		return err
	}
	return converter.ConvertToHub(spokeObj, obj)
}

func (c conversionSubResourceClient) Update(ctx context.Context, obj client.Object, opts ...client.SubResourceUpdateOption) error {
	gvk, err := c.GroupVersionKindFor(obj)
	if err != nil {
		return err
	}
	...
	if !conversionRequired(gvk) {
		return c.internalClient.Status().Update(ctx, obj, opts...)
	}
...
func (c conversionSubResourceClient) Patch(ctx context.Context, obj client.Object, patch client.Patch, opts ...client.SubResourcePatchOption) error {
	...
	spokeObj, err := newClientObject(c.Scheme(), converter.SpokeGroupVersionKind())
	if err != nil {
		return err
	}
	if err := converter.ConvertFromHub(obj, spokeObj); err != nil {
...
func converterFor(gvk schema.GroupVersionKind, preferredVersion string) (conversion.ConvertibleWrapper, error) {
	...
	case vmoprv1alpha2.GroupVersion.Version:
		...
		case vmoprvhub.GroupVersion.WithKind("VirtualMachineGroup"):
			return &vmoprv1alpha2conversion.VirtualMachineGroupConvertibleWrapper{}, nil
	...
	case vmoprv1alpha5.GroupVersion.Version:
		...
		case vmoprvhub.GroupVersion.WithKind("VirtualMachineGroup"):
			return &vmoprv1alpha5conversion.VirtualMachineGroupConvertibleWrapper{}, nil
		case vmoprvhub.GroupVersion.WithKind("VirtualMachineImage"):
