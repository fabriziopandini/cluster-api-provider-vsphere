...
CAPI_HACK_TOOLS_VER := bae425233190b4e365c7b285bf07a58516813fcb # Note: this the commit ID of CAPI main from 20251222103634-bae425233190.
...
VM_OPERATOR_COMMIT ?= a9e1dfbc0045d9abf50cc281a2a8843e9a1e1314
VM_OPERATOR_VERSION ?= v1.9.0-417-ga9e1dfbc
...
generate-go-deepcopy: $(CONTROLLER_GEN) ## Generate deepcopy go code for core
	...
	$(CONTROLLER_GEN) \
		...
		paths=./apis/... \
		paths=./pkg/services/conversion/api/...
	$(CONTROLLER_GEN) \
    	object:headerFile=./hack/boilerplate/boilerplate.generatego.txt \
    	paths=./$(VCSIM_DIR)/api/...
...
generate-manifests-vm-operator-v1.9.0-417-ga9e1dfbc:
	@cd "$(ROOT_DIR)/$(VM_OPERATOR_TMP_DIR)"; \
	make kustomize-wcp
	@cd "$(ROOT_DIR)"
...
docker-vm-operator-build-%:
	$(MAKE) ARCH=$* docker-build-vm-operator-$(VM_OPERATOR_VERSION)

.PHONY: docker-build-vm-operator
docker-build-vm-operator: checkout-vm-operator
	@if [ -z "${VM_OPERATOR_VERSION}" ]; then echo "VM_OPERATOR_VERSION is not set"; exit 1; fi
	$(MAKE) ARCH=$(ARCH) docker-build-vm-operator-$(VM_OPERATOR_VERSION)
...
docker-build-vm-operator-vm-operator-v1.9.0-417-ga9e1dfbc:
	@if [ -z "${VM_OPERATOR_IMAGE_TAG}" ]; then echo "VM_OPERATOR_IMAGE_TAG is not set"; exit 1; fi
	cd "$(ROOT_DIR)/$(VM_OPERATOR_TMP_DIR)"; \
