...
func acquire(ctx context.Context, client *boskos.Client, resourceType string) error {
	...
	if !strings.HasPrefix(folder.(string), allowedFolderPrefix) {
		return fmt.Errorf("refusing to acquire resource %q: folder %q is outside the allowed prefixes %v", res.Name, folder, allowedFolderPrefix)
	}
	if !strings.HasPrefix(resourcePool.(string), allowedResourcePoolPrefix) {
		return fmt.Errorf("refusing to acquire resource %q: resourcePool %q is outside the allowed prefixes %v", res.Name, resourcePool, allowedResourcePoolPrefix)
	}
...
func release(ctx context.Context, client *boskos.Client, resourceName, vSphereUsername, vSpherePassword, vSphereServer, vSphereTLSThumbprint, vSphereFolder, vSphereResourcePool string) error {
	...
	if !strings.HasPrefix(vSphereFolder, allowedFolderPrefix) {
		return fmt.Errorf("refusing to release resource %q: folder %q is outside the allowed prefixes %v", resourceName, vSphereFolder, allowedFolderPrefix)
	}
	if !strings.HasPrefix(vSphereResourcePool, allowedResourcePoolPrefix) {
		return fmt.Errorf("refusing to release resource %q: resourcePool %q is outside the allowed prefixes %v", resourceName, vSphereResourcePool, allowedResourcePoolPrefix)
	}
